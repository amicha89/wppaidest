    ...
    public function delete($id)
    {
        ...
        if (!empty($currency)) {
            ...
            if (module('CryptoExchange')) { 
                ...
                $direction = \Modules\CryptoExchange\Entities\ExchangeDirection::where('from_currency_id', $currency->id)->orWhere('to_currency_id', $currency->id)->first();
                
                if (isset($direction)) {
                    $this->helper->one_time_message('error',  __('Crypto Direction already exist for this currency.')  );
                    return redirect(Config::get('adminPrefix').'/settings/currency');
                }
            }
