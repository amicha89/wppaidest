          ...
          <div class="box box-info">
              ...
              <form action="{{ url(\Config::get('adminPrefix').'/save-preference') }}" method="post" id="preference-form" class="form-horizontal">
                ...
                <div class="box-body">
                  ...
                  {{-- Filesize --}}
        					<div class="form-group">
        					  <label class="col-sm-4 control-label" for="file_size">{{ __('Max Filesize') }}</label>
        					  <div class="col-sm-6">
        					    <input type="text" name="file_size" class="form-control" value="{{ $prefData['preference']['file_size'] ?? '' }}" max="20" placeholder="Max file upload size in MB (eg - 2MB)" id="file_size">
        					  	<span class="text-danger">{{ $errors->first('file_size') }}</span>
                      <small class="form-text text-muted"><strong>{{ __('The value must be less than or equal to 20 MB') }}</strong></small>
        					  </div>
        					</div>
