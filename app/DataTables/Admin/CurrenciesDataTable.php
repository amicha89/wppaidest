...
class CurrenciesDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($currency) {
                ...
                if ($currency->type == 'crypto_asset') {
                    $edit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(Config::get('adminPrefix').'/blockio/edit/' . encrypt($currency->code)) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                } else {
                ...
                $delete = (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_currency')) && (($currency->type == 'fiat') || ($currency->type == 'crypto'))  ? '<a href="' . url(Config::get('adminPrefix').'/settings/delete_currency/' . $currency->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;' : '';
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'name', 'name' => 'cryptoAssetSetting.cryptoProvider.name', 'title' => 'Name', 'visible' => false])
