...
class CurrencyExchange extends Model
{
    ...
    public function getExchangesListForCsvExport($from, $to, $status, $currency, $user)
    {
        ...
        if (! empty($date_range)) {
            $currencyExchanges->whereDate('currency_exchanges.created_at', '>=', $from)->whereDate('currency_exchanges.created_at', '<=', $to)->select('currency_exchanges.*');
        }
        else {
            $currencyExchanges->select('currency_exchanges.*');
        }
