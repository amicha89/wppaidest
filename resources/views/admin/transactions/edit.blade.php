                                ...
                                <div class="panel-body">
                                    <div class="mt-4 p-4 bg-secondary rounded shadow">
                                        ...
                                        @if (module('BlockIo'))
                                            ...
                                            @if (isset($txId))
                                                <div class="form-group">
                                                    <label class="control-label col-sm-3" for="crypto_txid">{{ $transaction->payment_method->name }} {{ __('TxId') }}</label>
                                        ...
                                        @endif
                                         ...
                                         @if(module('CryptoExchange') && ($transaction->transaction_type_id == Crypto_Exchange || $transaction->transaction_type_id == Crypto_Buy || $transaction->transaction_type_id == Crypto_Sell ))
                                            ...
                                            @if(isset($receiving_details) && !empty($receiving_details))
                                                <div class="form-group">
                                                    <label class="control-label col-sm-3" for="transactions_uuid">{{ __('Receiving Details') }}</label>
                                                    <div class="col-sm-9">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-9">
                                                        ...
                                                        @elseif ($transaction->transaction_type_id == Crypto_Sent || $transaction->transaction_type_id == Crypto_Received)
                                                            <p class="form-control-static"><span class="label label-danger" id="crypto-sent-status" style="white-space: unset !important;">{{ str_replace('_', ' ', $transaction->transaction_type->name) }}{{ __(' Status Cannot Be Changed') }}</span></p>
                                                        @else
                                    ...
                                    <div class="mt-4 p-4 bg-secondary rounded shadow">
                                        ...
                                        <div class="form-group total-deposit-feesTotal-space">
                                            @if (module('BlockIo') && optional($transaction->payment_method)->id == BlockIo && optional($transaction->currency)->type == 'crypto_asset')
                                                <label class="control-label col-sm-6" for="fee">{{ __('Network Fee') }}</label>
                                            @else
                                                ...
                                                @if (module('BlockIo') && $transaction->currency->type == 'crypto_asset')
                                                    <p class="form-control-static">
                                                        @if ($transaction->transaction_type_id == Crypto_Sent)
                                                            {{ moneyFormat($transaction->currency->symbol, formatNumber($network_fee, $transaction->currency_id)) }}
                                                        @elseif ($transaction->transaction_type_id == Crypto_Received)
                                                            {{ moneyFormat($transaction->currency->symbol, formatNumber(0.00000000, $transaction->currency_id)) }}
                                                        @endif
                                                    </p>
                                                @else
                                                ...
                                                <div class="col-sm-6">
                                                    <p class="form-control-static">
                                                        @if (module('BlockIo') && ($transaction->currency->type == 'crypto_asset') && ($transaction->transaction_type_id == Crypto_Sent))
                                                            {{ moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal + $network_fee, $transaction->currency_id)) }}
                                                        @else
                                                            {{  moneyFormat($transaction->currency->symbol, str_replace("-",'',formatNumber($transaction->total, $transaction->currency->id)) ) }}
                                                        @endif
                                                    </p>
                                                </div>
                                            </div>
