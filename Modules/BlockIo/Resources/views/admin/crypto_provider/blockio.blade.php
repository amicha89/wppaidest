            ...
            <ul class="nav nav-tabs cus" role="tablist">
                @foreach ($providers as $cryptoProvider)
                    @if (isset($activeCryptoProviders[$cryptoProvider->alias]) && $activeCryptoProviders[$cryptoProvider->alias])
                        <li class="{{ $cryptoProvider->alias == $selected_provider ? 'active' : '' }}">
                            <a href="{{ route('admin.crypto_providers.list', $cryptoProvider->alias) }}">{{ $cryptoProvider->alias }}</a>
                        </li>
                    @endif
                @endforeach
            </ul>
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="d-flex add-search">
                    ...
                    @if (Common::has_permission(\Auth::guard('admin')->user()->id, 'add_crypto_asset'))
                        <a href="{{ route('admin.blockio_asset.create') }}" class="btn btn-theme pull-right"><span class="fa fa-plus"></span>&nbsp;{{ __('Add New Asset') }}</a>
                    @endif
                </div>
            </div>
        </div>
                                        ...
                                        @foreach ($provider->cryptoAssetSettings as $cryptoAsset)
                                            ...
                                            <div class="crypto-card">
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu pull-right crypto-dropdown-list">
                                                        @if (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_crypto_asset'))
                                                            <li>
                                                                <a href="{{ url(Config::get('adminPrefix') . '/' . strtolower($provider->name) . '/edit/' . encrypt($cryptoAsset->network)) }}">{{ __('Edit') }}</a>
                                                            </li>
                                                        @endif
                                                        <li>
                                                            <a href="#" class="network" data-network={{ encrypt($cryptoAsset->network) }}  data-status={{ $cryptoAsset->status == 'Active' ? 'Inactive' : 'Active' }}  >{{ $cryptoAsset->status == 'Active' ? __('Mark as Inactive') : __('Mark as Active')}}</a>
                                                        </li>
                                                        <li>
                                                            <a href="#" data-toggle="modal" data-target="#addressValidationModal" data-network={{ $cryptoAsset->network }} class="validate-network">{{ __('Validate Address') }}</a>
                                                        </li>
                                                        @if (!str_contains($cryptoAsset->network, 'TEST'))
                                                        <li>
                                                            <a href="{{ url(Config::get('adminPrefix') . '/' . strtolower($provider->name) . '/current-price-list/' . encrypt($cryptoAsset->network)) }}">{{ __('Current Price') }}</a>
                                                        </li>
                                                        @endif
                                                    </ul>
                                                </div>
                    ...
                    <div class="modal-footer">
                        <div class="row">
                            <div class="col-md-12 text-center">
                                <button type="submit" class="btn btn-theme" id="address-validate-button"><i class="fa fa-spinner fa-spin d-none"></i>
